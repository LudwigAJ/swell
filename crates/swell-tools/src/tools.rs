    ...
    fn validate_path(&self, path: &Path) -> Result<(), SwellError> {
        ...
        if let Some(workspace) = &self.workspace_path {
            let canonical_workspace = workspace.canonicalize()
                .map_err(|e| SwellError::ToolExecutionFailed(format!("Cannot resolve workspace: {}", e)))?;
            
            if !canonical_path.starts_with(&canonical_workspace) {
                return Err(SwellError::ToolExecutionFailed(format!(
                    "Path '{}' is outside workspace boundaries", path.display()
                )));
            }
        }
        Ok(())
...
impl Tool for WriteFileTool {
    ...
    async fn execute(&self, arguments: serde_json::Value) -> Result<ToolOutput, SwellError> {
        ...
        let temp_file = NamedTempFile::new_in(temp_dir)
            .map_err(|e| SwellError::ToolExecutionFailed(format!("Failed to create temp file: {}", e)))?;
        ...
        match persist_result {
            Ok(_) => {
                ...
                Ok(ToolOutput {
                    success: true,
                    result: format!("Wrote {} bytes to {} atomically", args.content.len(), args.path),
                    error: None,
                })
            }
            Err(e) => {
...
impl Tool for ShellTool {
    ...
    fn description(&self) -> String { "Execute a shell command with timeout support".to_string() }
    ...
    async fn execute(&self, arguments: serde_json::Value) -> Result<ToolOutput, SwellError> {
        ...
        let timeout_duration = Duration::from_secs(args.timeout_secs.unwrap_or(self.default_timeout_secs));
        ...
        match result {
            ...
            Err(_) => {
                // Timeout
                Err(SwellError::ToolExecutionFailed(format!(
                    "Command timed out after {} seconds", timeout_duration.as_secs()
                )))
            }
        }
    }
...
impl GitTool {
    ...
    async fn git_status(&self, cwd: &Path) -> Result<ToolOutput, SwellError> {
        let output = self.run_git(&["status", "--porcelain", "-b"], Some(cwd)).await?;
        ...
        if let Some(branch_line) = lines.first() {
            ...
            if let Some(branch) = branch_line.strip_prefix("## ") {
                status_info["branch"] = serde_json::json!(branch.split_whitespace().next().unwrap_or(""));
            }
        }

        let is_dirty = lines.len() > 1 || lines.first().map(|l| !l.starts_with("##")).unwrap_or(false);
        ...
        for line in lines.iter().skip(1) {
            if line.len() >= 3 {
                ...
                if staged != " " && staged != "?" {
                    status_info["staged"].as_array_mut().unwrap().push(serde_json::json!({
                        "status": staged,
                        "path": path
                    }));
                }
                if modified != " " {
                    status_info["modified"].as_array_mut().unwrap().push(serde_json::json!({
                        "status": modified,
                        "path": path
                    }));
                }
    ...
    async fn git_commit(&self, message: &str, metadata: Option<&str>, cwd: &Path) -> Result<ToolOutput, SwellError> {
        ...
        self.run_git(&["commit", "-m", &full_message], Some(cwd)).await
    }
    ...
    async fn create_branch(&self, branch_name: &str, cwd: &Path) -> Result<ToolOutput, SwellError> {
        self.run_git(&["checkout", "-b", branch_name], Some(cwd)).await
    }
}
...
impl Tool for GitTool {
    ...
    fn description(&self) -> String { "Execute git operations (status, diff, log, commit, branch)".to_string() }
    fn risk_level(&self) -> ToolRiskLevel { ToolRiskLevel::Write }
    fn permission_tier(&self) -> PermissionTier { PermissionTier::Ask }
    ...
    async fn execute(&self, arguments: serde_json::Value) -> Result<ToolOutput, SwellError> {
        ...
        match args.operation.as_str() {
            ...
            "commit" => {
                ...
                self.git_commit(&message, args.metadata.as_deref(), cwd).await
            }
            "branch" => {
                if let Some(branch_name) = args.args.first() {
                    self.create_branch(branch_name, cwd).await
                } else {
                    Err(SwellError::ToolExecutionFailed("Branch name required".to_string()))
                }
            }
            _ => Err(SwellError::ToolExecutionFailed(format!(
                "Unknown git operation: {}", args.operation
            ))),
        }
    }
}
...
impl FileEditTool {
    pub fn new() -> Self {
        Self { max_diff_size: 1_000_000 } // 1MB default
    }
    ...
    fn generate_diff(&self, path: &str, old_content: &str, new_content: &str) -> String {
        ...
        for i in 0..max_lines {
            ...
            match (old_line, new_line) {
                ...
                (Some(old), Some(new)) => {
                    if !in_hunk {
                        writeln!(&mut diff_output, "@@ -{},{} +{},{} @@", 
                            line_num, old_lines.len().saturating_sub(i),
                            line_num, new_lines.len().saturating_sub(i)).ok();
                        in_hunk = true;
                    }
...
impl SearchTool {
    ...
    /// Execute grep search
    async fn grep(&self, pattern: &str, path: &str, case_insensitive: bool) -> Result<ToolOutput, SwellError> {
...
impl Tool for SearchTool {
    ...
    async fn execute(&self, arguments: serde_json::Value) -> Result<ToolOutput, SwellError> {
        ...
        match args.operation.as_str() {
            ...
            _ => Err(SwellError::ToolExecutionFailed(format!(
                "Unknown search operation: {}", args.operation
            ))),
        }
    }
}
...
mod tests {
    ...
    async fn test_read_file_tool_workspace_validation() {
        ...
        let tool = ReadFileTool::new()
            .with_workspace_path("/tmp/nonexistent");
    ...
    async fn test_search_tool_glob() {
        let dir = tempdir().unwrap();
        let _file1 = tokio::fs::write(dir.path().join("test1.txt"), "content").await.unwrap();
        let _file2 = tokio::fs::write(dir.path().join("test2.txt"), "content").await.unwrap();
